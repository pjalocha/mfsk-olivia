...
int CountDiffs(char *Input, char *Output, int Len)
{ int Idx;
  int Diffs=0;
...
void AddNoise(float *Data, int Len, float RMS=1.0)
{ int Idx;
  Cmpx<float> Noise;
  for(Idx=0; Idx<Len; Idx++)
...
*/
  ...
  Parameters.ReadOption("-M4");
  Parameters.ReadOption("-I16");
  ...
  for(Idx=0; Idx<((MessageLen/5+20+20)*64); Idx++)
  {
  ...
  char OutputMessage[MessageLen+256+1];
  ...
  int Ofs;
  int MinOfs=0; int MinDiffs=MessageLen;
  for(Ofs=0; (Ofs+MessageLen)<OutputMessageLen; Ofs++)
  { int Diffs=CountDiffs(InputMessage,OutputMessage+Ofs,MessageLen);
    // printf(" %d",Diffs);
